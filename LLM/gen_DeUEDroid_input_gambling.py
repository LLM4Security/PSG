...
# 指定目录的路径
directory_path = "/mnt/inside_15T/PPG_dataset/DeUEDroid_result/Gambling/"
...
app_processing_log = './Gambling_time.log'
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        for i in page_list.keys():
            file_list.append('/mnt/inside_15T/PPG_dataset/DeUEDroid_template/Gambling/'+subdirectory+'/'+i)
        ...
        for file in tqdm(file_list):
            ...
            for i in doc_result.keys():
                ...
                try:
                    ...
                    # Optionally, print the error message to the console
                    print(error_message)
            if not os.path.exists('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Gambling/'+subdirectory):
                os.makedirs('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Gambling/'+subdirectory)
            with open('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Gambling/'+subdirectory+'/'+file.replace('/mnt/inside_15T/PPG_dataset/DeUEDroid_template/Gambling/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
