...
# 指定目录的路径
directory_path = "/mnt/inside_15T/PPG_dataset/DeUEDroid_result/Normal/"

# 获取目录下的所有子目录
subdirectories = get_subdirectories(directory_path)

subdirectories = subdirectories[:len(subdirectories)/2]
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        for i in page_list.keys():
            file_list.append('/mnt/inside_15T/PPG_dataset/DeUEDroid_template/Normal/'+subdirectory+'/'+i)
        ...
        for file in tqdm(file_list):
            ...
            for i in doc_result.keys():
                ...
                try:
                    ...
                    # Optionally, print the error message to the console
                    print(error_message)
            if not os.path.exists('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory):
                os.makedirs('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory)
            with open('/mnt/inside_15T/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory+'/'+file.replace('/mnt/inside_15T/PPG_dataset/DeUEDroid_template/Normal/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
