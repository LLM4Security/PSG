...
tokenizer = AutoTokenizer.from_pretrained("/home/jiuyan/chatglm3-6b", trust_remote_code=True)
model = AutoModel.from_pretrained("/home/jiuyan/chatglm3-6b", trust_remote_code=True, device='cuda')
model = model.eval()

def gen_func_abs(func_name, doc_tem, visited, apk):
    temp_result = {}
    path = '/home/jiuyan/PPG/AndroidBench_function/'+apk+'/.home.jiuyan.PPG.AndroidBench_decompile.'+apk+'.sources.'+func_name
    if os.path.exists(path):
        if func_name in doc_tem.keys():
            ...
            for i in func_names:
                if os.path.exists('/home/jiuyan/PPG/AndroidBench_function/'+apk+'/.home.jiuyan.PPG.AndroidBench_decompile.'+apk+'.sources.'+i):
                    if i not in visited:
                        visited.add(i)
                        print('add code sub:'+i)
                        with open('/home/jiuyan/PPG/AndroidBench_function/'+apk+'/.home.jiuyan.PPG.AndroidBench_decompile.'+apk+'.sources.'+i, 'r', encoding='utf-8') as f:
                            add_code = f.read()
                        code = code + '\n' + add_code
                        if i in doc_tem.keys():
                            sub_funcs = doc_tem[i].split('||')
                            for j in sub_funcs:
                                if os.path.exists('/home/jiuyan/PPG/AndroidBench_function/'+apk+'/.home.jiuyan.PPG.AndroidBench_decompile.'+apk+'.sources.'+j):
                                    if j not in visited:
                                        visited.add(j)
                                        print('generate code sub'+j)
                                        temp_result[j] = gen_func_abs(j, doc_tem, visited, apk)  
...
subdirectories = subdirectories[15:]
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        try:
            ...
            for i in page_list.keys():
                file_list.append('/home/jiuyan/PPG/AndroidBench_template/'+subdirectory+'/'+i)
            ...
            for file in tqdm(file_list):
                ...
                for i in doc_result.keys():
                    ...
                    try:
                        ...
                        print(error_message)
                if not os.path.exists('/home/jiuyan/PPG/AndroidBench_batch2_codesummary/'+subdirectory):
                    os.makedirs('/home/jiuyan/PPG/AndroidBench_batch2_codesummary/'+subdirectory)
                with open('/home/jiuyan/PPG/AndroidBench_batch2_codesummary/'+subdirectory+'/'+file.replace('/home/jiuyan/PPG/AndroidBench_template/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
