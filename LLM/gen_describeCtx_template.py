...
# 指定目录的路径
directory_path = "/mnt/inside_15T/PPG_dataset/DescribeCTX_result"
        ...
        if os.path.exists(path):
            ...
            for i in file_list:
                with open(path + '/' + i,'r') as f:
                    dot_graph = f.read()
                if not os.path.exists('/mnt/inside_15T/PPG_dataset/describeCtx_template/'+subdirectory):
                    os.makedirs('/mnt/inside_15T/PPG_dataset/describeCtx_template/'+subdirectory)
                try:
                    with open('/mnt/inside_15T/PPG_dataset/describeCtx_template/'+subdirectory+'/'+i[:-4],'w') as output_file:
