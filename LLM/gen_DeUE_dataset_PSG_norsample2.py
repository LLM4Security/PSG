...
logging.basicConfig(filename='error3_log.txt', level=logging.ERROR)
...
def gen_func_abs(func_name, doc_tem, visited, apk, start_time, time_limit):
    ...
    if time.time() - start_time > time_limit:
        return func_name
    path = '/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_function/'+apk+'/'+func_name
    if os.path.exists(path):
        if func_name in doc_tem.keys():
            ...
            for i in func_names:
                if os.path.exists('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_function/'+apk+'/'+i):
                    if i not in visited:
                        visited.add(i)
                        print('add code sub:'+i)
                        with open('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_function/'+apk+'/'+i, 'r', encoding='utf-8') as f:
                            add_code = f.read()
                        code = code + '\n' + add_code
                        if i in doc_tem.keys():
                            ...
                            for j in sub_funcs:
                                if time.time() - start_time > time_limit:
                                    break
                                if os.path.exists('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_function/'+apk+'/'+j):
                                    if j not in visited:
                                        visited.add(j)
...
subdirectories = subdirectories[int(len(subdirectories)/2)+67+28+73+67+21+22+70:]

time_limit_per_app = 20*60

app_processing_log = './Normal2_time.log'
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        for i in page_list.keys():
            file_list.append('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_template/Normal/'+subdirectory+'/'+i)
        ...
        for file in tqdm(file_list):
            ...
            for i in doc_result.keys():
                ...
                try:
                    ...
                    # Optionally, print the error message to the console
                    print(error_message)
            if not os.path.exists('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory):
                os.makedirs('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory)
            with open('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory+'/'+file.replace('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_template/Normal/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
