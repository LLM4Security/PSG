...
def gen_func_abs(func_name, doc_tem, visited, apk, start_time, time_limit):
    ...
    if time.time() - start_time > time_limit:
        return func_name
    path = '/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_function/'+apk+'/.'+func_name
    if os.path.exists(path):
        if func_name in doc_tem.keys():
            ...
            for i in func_names:
                if os.path.exists('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_function/'+apk+'/.'+i):
                    if i not in visited:
                        visited.add(i)
                        # print('add code sub:'+i)
                        with open('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_function/'+apk+'/.'+i, 'r', encoding='utf-8') as f:
                            add_code = f.read()
                        code = code + '\n' + add_code
                        if i in doc_tem.keys():
                            ...
                            for j in sub_funcs:
                                if time.time() - start_time > time_limit:
                                    break
                                if os.path.exists('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_function/'+apk+'/.'+j):
                                    if j not in visited:
                                        visited.add(j)
                                        # print('generate code sub'+i)
                                        temp_result[j] = gen_func_abs(j, doc_tem, visited, apk, start_time, time_limit)  
...
subdirectories = get_subdirectories('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_result')

time_limit_per_app = 20*60

app_processing_log = './2FA_time.log'

subdirectories = subdirectories[60:400]
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        for i in page_list.keys():
            file_list.append('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_template/'+subdirectory+'/'+i)
        ...
        for file in tqdm(file_list):
            ...
            for i in doc_result.keys():
                ...
                try:
                    ...
                    # Optionally, # print the error message to the console
                    print(error_message)
            if not os.path.exists('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_abstract/'+subdirectory):
                os.makedirs('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_abstract/'+subdirectory)
            with open('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_abstract/'+subdirectory+'/'+file.replace('/cpfs01/projects-SSD/cfff-5822cced0fd0_SSD/cxy_22210240138/PSG/PSG/backdoor/2FA_template/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
