...
tokenizer = AutoTokenizer.from_pretrained("/mnt/maldect_ssd_2T/chatglm3-6b32k", trust_remote_code=True)
model = AutoModel.from_pretrained("/mnt/maldect_ssd_2T/chatglm3-6b32k", trust_remote_code=True, device='cuda')
model = model.eval()
...
# 指定目录的路径
directory_path = "/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_result/Normal"

subdirectories = get_subdirectories(directory_path)
subdirectories = subdirectories[int(len(subdirectories)/2)+121+190+95:]
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        path = '/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Normal/'+subdirectory
            ...
            for i in tqdm(ui_dic.keys()):
                ...
                if os.path.exists(path+'/'+i):
                    try:
                        ...
                        llm_input = f'This is a page called {page_title}, which has the following lifecycle events:\n'
                        ...
                        print(error_message)
                if not os.path.exists('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_page/Normal/'+subdirectory):
                    os.makedirs('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_page/Normal/'+subdirectory)
                with open('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_page/Normal/'+subdirectory+'/page.json','w', encoding='utf-8') as f:
                    json.dump(result,f)