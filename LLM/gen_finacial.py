...
# 指定目录的路径
directory_path = "/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_result/Financial/"

# 获取目录下的所有子目录
subdirectories = get_subdirectories(directory_path)

subdirectories = subdirectories[50:]

time_limit_per_app = 20*60

app_processing_log = './Financial_time.log'
...
for subdirectory in tqdm(subdirectories):
    ...
    if os.path.exists(path+'uiHtml.json') and os.path.exists(path+'/cfgs'):
        ...
        for i in page_list.keys():
            file_list.append('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_template/Financial/'+subdirectory+'/'+i)
        ...
        for file in tqdm(file_list):
            ...
            for i in doc_result.keys():
                ...
                try:
                    ...
                    # Optionally, print the error message to the console
                    print(error_message)
            if not os.path.exists('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Financial/'+subdirectory):
                os.makedirs('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Financial/'+subdirectory)
            with open('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_abstract/Financial/'+subdirectory+'/'+file.replace('/mnt/maldect_ssd_2T/PPG_dataset/PPG_dataset/DeUEDroid_template/Financial/'+subdirectory+'/', ''), 'w', encoding='utf-8') as f:
